...
test("should render Main Page correctly", () => {
  ...
  expect(
    getByText(/Please submit a link to get started!/i)
  ).toBeInTheDocument();
});

test("should show data on Main Page correctly", () => {
  const mockLink = [
    ["hepsiburada", "www.hepsiburada.com", 5],
    ["n11", "www.n11.com", 2],
  ];
...
test("should ascending-sort data on Options correctly", () => {
  ...
  const wrapper = shallow(
    <Main options={mockLink} onSortChange={onSortChange} filters={"Desc"} />
  );
...
test("should ascending-sort data on Options correctly", () => {
  ...
  const wrapper = shallow(
    <Main options={mockLink} onSortChange={onSortChange} filters={"Asc"} />
  );
